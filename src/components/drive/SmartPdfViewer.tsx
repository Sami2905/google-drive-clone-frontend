...
export default function SmartPdfViewer({ url }: { url: string }) {
  const [scale, setScale] = React.useState(1);
  const [rotation, setRotation] = React.useState(0);
  const [viewerType, setViewerType] = React.useState<'iframe' | 'object' | 'error'>('iframe');
  ...
  const handleObjectError = () => {
    console.log('Object tag failed, showing error');
    setViewerType('error');
    setIsLoading(false);
  };
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="grid place-items-center overflow-auto rounded-lg border p-2">
        ...
        >
          {isLoading && viewerType !== 'error' && (
            <div className="absolute inset-0 flex items-center justify-center bg-gray-50 dark:bg-gray-900">
              <div className="text-center">
                <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-600 mx-auto mb-2"></div>
                <p className="text-sm text-gray-600 dark:text-gray-400">
                  Loading PDF... {viewerType === 'object' && '(fallback mode)'}
                </p>
                {retryCount > 0 && (
